       ...
       for l in range(loops):
              for index, row in df.iterrows():
                     if row['anomaly'] == 0:
                            ...
                            if 1 in df['anomaly'][start_index:index].values and 1 in df['anomaly'][
                                                                                     index + 1:end_index].values:
                                   window = df['anomaly'][start_index:end_index]
...
for sfa_control_area in sfa_control_areas:
       factory.add_SFA(n_sfa_components, selected_sfa_features, poly_degree=sfa_degree, control_area=sfa_control_area, batch_size=200, cascade_length=1)
